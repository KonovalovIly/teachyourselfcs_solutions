...
int howManyBits(int x) {
  ...
  int sign = x >> 31;
  int n = x ^ sign; // Для отрицательных: инвертируем биты (x ^ 0xFFFFFFFF)

  // Бинарный поиск старшего бита
  bit16 = (!!(n >> 16)) << 4;
  n = n >> bit16;
...
int floatFloat2Int(unsigned uf) {
  int E, mantissa, exp, value;
  int sign = uf >> 31;
  exp = (uf >> 23) & 0xFF;
  mantissa = uf & 0x007FFFFF;

  // Special cases
  if (exp == 0xFF) return 0x80000000; // NaN or Infinity
  if (!exp) return 0;                 // Denormalized or zero

  E = exp - 127;
  value = mantissa | 0x00800000;  // Add implicit 1

  if (E < 0) return 0;                // |f| < 1.0
  if (E > 31) return 0x80000000;      // Overflow

  // Adjust mantissa
  if (E < 23) {
      value >>= (23 - E);             // Round toward zero
  } else {
    value <<= (E - 23);             // Shift left
  }
...
unsigned floatPower2(int x) {
  int shift, exp;
  if (x > 127) { // 2^128 и больше -> INF
      return 0x7F800000;
  }
  if (x < -149) { // Меньше 2^-149 -> 0
      return 0;
  }
