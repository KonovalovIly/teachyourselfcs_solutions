		...
		int argc, char *argv[])
{
    sim_interp = interp;
    if (argc != 1) {
    Tcl_SetObjResult(interp, Tcl_NewStringObj("No arguments allowed", -1));
	return TCL_ERROR;
    }
    sim_reset();
    if (post_load_mem) {
    ...
    Tcl_SetObjResult(interp, Tcl_NewStringObj(stat_name(status), -1));
    return TCL_OK;
}
		   ...
		   int argc, char *argv[])
{
    ...
    sim_interp = interp;
    if (argc != 2) {
    Tcl_SetObjResult(interp, Tcl_NewStringObj("One argument required", -1));
	return TCL_ERROR;
    }
    object_file = fopen(argv[1], "r");
    if (!object_file) {
	sprintf(tcl_msg, "Couldn't open code file '%s'", argv[1]);
    Tcl_SetObjResult(interp, Tcl_NewStringObj(tcl_msg, -1));
	return TCL_ERROR;
    }
    sim_reset();
	      ...
	      int argc, char *argv[])
{
    ...
    sim_interp = interp;
    if (argc > 2) {
    Tcl_SetObjResult(interp, Tcl_NewStringObj("At most one argument allowed", -1));
	return TCL_ERROR;
    }
    if (argc >= 2 &&
    ...
    Tcl_SetObjResult(interp, Tcl_NewStringObj(stat_name(run_status), -1));
    return TCL_OK;
}
