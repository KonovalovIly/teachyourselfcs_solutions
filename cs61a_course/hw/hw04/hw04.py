...
def balanced(m):
    ...
    if is_mobile(m):
        ...
        torque_equal = weight(left(m)) == weight(right(m))
        return torque_equal and balanced(left_arm) and balanced(right_arm)
...
def weight(m):
    if is_planet(m):
        return mass(m)
    if is_arm(m):
        return length(m) * weight(end(m))
    if is_mobile(m):
        return just_mass(left(m)) + just_mass(right(m))

def just_mass(m):
    if is_planet(m):
        return mass(m)
    if is_arm(m):
        return just_mass(end(m))
    if is_mobile(m):
        return just_mass(left(m)) + just_mass(right(m))
