...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        lambda_name = expressions.first.first
        params = expressions.first.rest
        body = expressions.rest
        current = params
        while current != nil:
            if not scheme_symbolp(current.first):
                raise SchemeError('non-symbol: {0}'.format(current))
            current = current.rest
        env.define(lambda_name, LambdaProcedure(formals=params, body=body, env = env))
        return lambda_name
    else:
...
def do_and_form(expressions, env):
    ...
    res = True
    while current != nil:
        res = scheme_eval(current.first, env)
        if res is False:
            return res
        current = current.rest
    return res
