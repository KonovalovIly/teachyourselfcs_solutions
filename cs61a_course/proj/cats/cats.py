...
def about(subject):
    ...
    def find(word):
        lis = split(remove_punctuation(lower(word)))
        for i in subject:
            if (i in lis):
                return True
        return False
...
def accuracy(typed, source):
    ...
    for i in range(0, len(typed_words)):
        if (i >= len(source_words)):
            break
        first = typed_words[i]
        second = source_words[i]
        if(first == second):
            accurac += 1

    return accurac / len(typed_words) * 100.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for index, word in enumerate(word_list):
        ...
        if current_diff < min_diff:
            ...
            closest_word = word
            closest_index = index
        elif current_diff == min_diff and index < closest_index:
            closest_word = word
            closest_index = index
...
def furry_fixes(typed, source, limit):
    ...
    if typed[0] != source[0]:
        limit -= 1
    return furry_fixes(typed[1:], source[1:], limit) + (1 if typed[0] != source[0] else 0)
...
def report_progress(typed, source, user_id, upload):
    ...
    for index, word in enumerate(typed):
        result = accuracy(word, source[index])
        if(result == 100.0):
            acc += 1
        else:
            break
    progress = {
        "id": user_id,
        "progress": acc / len(source)
    }
    upload(progress)
    return acc / len(source)
...
def fastest_words(words_and_times):
    ...
    for word in word_indices:
        smallest_player = 0
        smallest = times[smallest_player][word]

        for i in player_indices:
            if (smallest > times[i][word]):
                smallest_player = i
                smallest = times[smallest_player][word]
        result[smallest_player] += [words[word]]
    return result
