class Account:
    ...
    def time_to_retire(self, amount):
        ...
        year_count = 0
        presumptive_balance = self.balance
        while presumptive_balance < amount:
            presumptive_balance += presumptive_balance * self.interest
            year_count += 1
        return year_count
...
class FreeChecking(Account):
    ...
    "*** YOUR CODE HERE ***"
    def withdraw(self, amount):
        if (self.current_withdraw >= 2):
            new_balance = super().withdraw(amount + self.withdraw_fee)
        else:
            self.current_withdraw += 1
...
def duplicate_link(s, val):
    ...
    while current is not Link.empty:
        if current.first == val:
            ...
            current = current.rest.rest  # skip the newly added node
        else:
