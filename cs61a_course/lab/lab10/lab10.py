...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        return exp
    elif exp in bindings:  # Variable lookup
        return bindings[exp]  # Get variable value from VARIABLES dict
...
def floor_div(args):
    ...
    """
    if args is nil:
        raise TypeError("floor_div requires at least one argument")

    first = calc_eval(args.first)

    if args.rest is nil:
        return first  # identity case: (// x) returns x

    second = calc_eval(args.rest.first)

    result = first // second

    remaining = args.rest.rest

    while remaining is not nil:
        next_val = calc_eval(remaining.first)
