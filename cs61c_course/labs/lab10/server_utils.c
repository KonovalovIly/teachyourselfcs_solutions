   ...
   *socket_number = socket(PF_INET, SOCK_STREAM, 0);
   ...
   while (1) {
      ...
      if (pid < 0) {
         ...
         close(client_socket_number); // Close the client socket if fork fails
         continue;
      } else if (pid == 0) { // Child process
         // Ensure the child process terminates if the parent dies
         int r = prctl(PR_SET_PDEATHSIG, SIGTERM);
         if (r == -1 || getppid() != parent_pid) {
            perror("Parent process died or prctl failed");
            close(client_socket_number); // Close the client socket
            exit(1);
         }

         // Handle the client request in the child process
         ...
         close(client_socket_number); // Close the client socket in the parent process
      }
#else
      dispatch(client_socket_number);
#endif
